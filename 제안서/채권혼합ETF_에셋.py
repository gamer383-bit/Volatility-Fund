...
ddF=(pd.Series(nav,index=dl)/pd.Series(nav,index=dl).cummax()-1)*100
ddB=(pd.Series(bm,index=dl)/pd.Series(bm,index=dl).cummax()-1)*100
ax2.fill_between(dl,ddB.values,color=GRAY,alpha=0.30,lw=0)
ax2.plot(dl,ddB.values,color=GRAY,lw=1.2,label=f'BM 드로우다운 (최대 {ddB.min():.1f}%)')
ax2.fill_between(dl,ddF.values,color=ORANGE,alpha=0.40,lw=0)
ax2.plot(dl,ddF.values,color=ORANGE,lw=1.4,label=f'펀드 드로우다운 (최대 {ddF.min():.1f}%)')
ax2.axhline(0,color='#8a97a8',lw=0.7)
ax2.set_ylabel('고점 대비 낙폭(%)',fontsize=9,color=NAVY)
ax2.set_ylim(min(ddB.min(),ddF.min())*1.15,2)
ax2.legend(fontsize=8.3,frameon=False,loc='lower left')
ax2.grid(alpha=0.2); ax2.spines[['top','right']].set_visible(False)
ax2.xaxis.set_major_formatter(mdates.DateFormatter('%y.%m'))
fig.text(0.99,0.01,'세로선=분기 리셋 · 하단=고점 대비 낙폭(드로우다운) 비교 · 매수 5bp/매도 30bp · 채권·현금 연 2.5% 가정 · 데이터: 기준 엑셀',ha='right',color=GRAY,fontsize=7.5)
fig.tight_layout()
...
WINDOWS=[('변동장 1년 ② (하락 왕복)','2024-05-10',365,'하락장에도 플러스 — 저가매수 수확','bt_bond_down.png'),
         ('변동장 1년 ① (보합 왕복)','2024-07-15',365,'등락 반복을 매매로 수확 → BM 초과','bt_bond_flat.png'),
         ('상승 1년','2023-08-30',365,'상승에 참여하며 등락 수확 병행','bt_bond_up.png'),
         ('하락 1년 (급락 구간)','2021-12-13',365,'깊은 하락, 낙폭을 1/4로 방어','bt_bond_down2.png')]
