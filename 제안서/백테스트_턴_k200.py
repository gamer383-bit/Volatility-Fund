...
END='2026-07-30'   # 16·17페이지 기준일 (사용자 지정: 매번 확인)
...
dts=ret.index
print(f"KOSPI200 지수(엑셀): {dts[0].date()} ~ {dts[-1].date()} ({len(ret)}영업일)")
...
draw_table('g','qpms_table_g_k2.png','상품1(성장형)')
draw_table('s','qpms_table_s_k2.png','상품2(안정형)')
...
for kind,color in (('g',ORANGE),('s',NAVY)):
    draw_win(kind,color,wd_i,'하락 1년',f'qpms_down_{kind}_k2.png',legend=True)
    draw_win(kind,color,up_i,'상승 1년',f'qpms_up_{kind}_k2.png')
    draw_win(kind,color,fl_i,'보합 1년',f'qpms_flat_{kind}_k2.png')
print("done")
