...
SIG=0.60; R=0.025; Q=0.025; T=1.0; TCB,TCS=0.0005,0.0030; K=100.0; nP=7000; W=1.10  # 복제비율 110%
def bs_put(S,K_,T_):
...
port=h*S+cash
payoff=-np.maximum(K-S,0.0)*W
hedgePnl=(port+tc)-payoff
pnlUnit=(P0*np.exp(R*T)-np.maximum(K-S,0.0))*W
Y=(pnlUnit-tc+hedgePnl)/100.0*100.0   # 구조화 수익(%) = 이론손익 − 비용 + 복제오차
X=(S/100.0-1.0)*100.0                  # 기초자산 등락률(%)
print(f"ATM풋 프리미엄 {P0:.2f}%(×110%={P0*W:.2f}%) · 평균수익 {Y.mean():+.2f}% · 수익확률 {(Y>=0).mean()*100:.0f}% · 손익분기 지수 약 {-(P0*np.exp(R*T)):.1f}%")
