...
- 기간: 기준가=2026-04-30 종가(=100), 노출 5월 첫 영업일 ~ 최신일 (만기 1년 지평)
- σ=직전 60영업일(주말 제외) 연율화(상한 60%) · r=q=2.5% · 매수 5bp/매도 30bp · 실거래일만
...
START='2026-05-01'; SIGCAP=0.60; WMIN=0.10   # 최근 3개월: 앵커 4/30 종가(5/1 휴장→노출 5/4부터)
...
def run(target_on):
    ...
    out={}
    for kind in ('g','s'):
        a0=i0-1                              # 앵커: START 직전 영업일 종가 (노출은 START부터)
...
def draw(dates,base,out,title,fname,show_restart):
    ...
    pos=[]
    for v,_ in labs:
        pos.append(v if not pos else min(v,pos[-1]-gap))
    if pos[-1]<lo+gap*0.3:              # 스택이 축 바닥 아래로 밀리면 전체를 위로
        d=(lo+gap*0.3)-pos[-1]
        pos=[p+d for p in pos]
    for (v,col),yy in zip(labs,pos):
