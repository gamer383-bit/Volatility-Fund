...
상품1(룰베이스): w = 150% − k×(S/S₀−1), 캡 [100,200], 기본 k=2 (레버리지ETF 미러)
상품2(옵션복제): w = 100% + ATM급 풋매도 복제비중(고정만기 T=0.25, 행사가는 w₀=150% 역산)
공통: 오픈형 → 리밸런싱일마다 기준(S₀/K) 리셋 → 편입비 150% 복귀. 스무딩은 트랜치 롤링.
출력: 편입비 곡선/테이블, ±10% 민감도(vs 레버리지ETF +20%p 순응), 3개월 예상수익 곡선(MC)
"""
...
def chart_weights(sig,tag,label):
    K2=calib_K(sig)
    S=np.linspace(60,140,161)
    fig,ax=plt.subplots(figsize=(7.6,5.6),dpi=110)
    ax.plot(S,w_model1(S,k=1.0)*100,color=CYAN, lw=2.0,ls='--',label='상품1 룰베이스 (k=1, 완만)')
    ax.plot(S,w_model1(S,k=2.0)*100,color=NAVY, lw=2.6,label='상품1 룰베이스 (k=2, 표준)')
    ...
    ax.set_xlim(60,140); ax.set_ylim(92,212)
    ax.set_xlabel('지수 (리밸런싱일=100)',fontsize=11,color=NAVY)
    ax.set_ylabel('편입비 (%)',fontsize=11,color=NAVY)
    ax.set_title(f'{label} — 지수대별 편입비 (상승 시↓ · 하락 시↑)',fontsize=13.5,color=NAVY,fontweight='bold',loc='left')
    ax.grid(alpha=0.25); ax.spines[['top','right']].set_visible(False)
    ax.legend(fontsize=10,frameon=False,loc='upper right')
    fig.text(0.99,0.01,f'상품2 행사가 K={K2:.1f} (시작 편입비 150% 역산) · r3%·q2% 가정',ha='right',color=GRAY,fontsize=8.5)
    fig.tight_layout(rect=[0,0.03,1,1])
    fig.savefig(os.path.join(IMG,f'open_w_{tag}.png')); plt.close(fig)
    # 테이블 출력
    rows=[70,80,85,90,95,100,105,110,115,120,130]
    print(f"\n[{label}] 편입비 테이블 (%)  · 상품2 K={K2:.1f}")
    print(f"{'지수':>5} | {'상품1 k=1':>9} | {'상품1 k=2':>9} | {'상품2 옵션복제':>12}")
    for m in rows:
        print(f"{m:5d} | {w_model1(np.array([m]),k=1.)[0]*100:9.1f} | {w_model1(np.array([m]),k=2.)[0]*100:9.1f} | {w_model2(np.array([m]),K2,sig)[0]*100:12.1f}")
    return K2

# ---------- ② ±10% 민감도 비교 ----------
def sensitivity(sig,K2,label):
    up1=(w_model1(np.array([110.]),k=2.)[0]-1.5)*100
    dn1=(w_model1(np.array([90.]),k=2.)[0]-1.5)*100
    up2=(w_model2(np.array([110.]),K2,sig)[0]-1.5)*100
    dn2=(w_model2(np.array([90.]),K2,sig)[0]-1.5)*100
    print(f"\n[{label}] ±10% 편입비 변동(150% 대비, %p): "
          f"레버리지ETF +10%시 +20/−10%시 −20(순응) | 상품1(k=2) {up1:+.0f}/{dn1:+.0f} | 상품2 {up2:+.1f}/{dn2:+.1f} (역방향)")
...
def chart_sensitivity(sens50,sens60):
    ...
    for b in list(b1)+list(b2):
        v=b.get_height()
        ax.text(b.get_x()+b.get_width()/2, v+(1.2 if v>=0 else -2.6), f'{v:+.0f}%p', ha='center',fontsize=10.5,fontweight='bold',
                color='#333')
...
def mc_quarter(sig,K2,nP=6000,days=63,tc=0.0015,seed=20260715):
    ...
    for t in range(days):
        ...
        S*=1.0+ret
        nw1=w_model1(S,k=2.0); nw2=w_model2(S,K2,sig)
...
def chart_mc(sig,K2,tag,label):
    ...
    for Y,col,nm in ((rB,'#9DB4CC','BM (150% 고정)'),(r1,NAVY,'상품1 룰베이스(k=2)'),(r2,ORANGE,'상품2 옵션복제')):
